...
local function snap_position(ent, status)
    if status.snap_position then
        ...
        if snap.x > 0 then
            offset.x = (-origin.x + pos.x + snap.x / 2) % snap.x - snap.x / 2
        end

        if snap.y > 0 then
            offset.y = (-origin.y + pos.y + snap.y / 2) % snap.y - snap.y / 2
        end

        if snap.z > 0 then
            offset.z = (-origin.z + pos.z + snap.z / 2) % snap.z - snap.z / 2
        end

        local absoffset = Vector(math.abs(offset.x), math.abs(offset.y), math.abs(offset.z))

        if absoffset.x > sensitivity.x then
            offset.x = offset.x - snap.x * (offset.x / absoffset.x)
        end

        if absoffset.y > sensitivity.y then
            offset.y = offset.y - snap.y * (offset.y / absoffset.y)
        end

        if absoffset.z > sensitivity.z then
            offset.z = offset.z - snap.z * (offset.z / absoffset.z)
        end
...
local function snap_angles(ent, status)
    if status.snap_angles then
        ...
        if snap then
            local offset = Angle((ang.p + snap.p / 2) % snap.p - snap.p / 2, (ang.y + snap.y / 2) % snap.y - snap.y / 2, (ang.r + snap.r / 2) % snap.r - snap.r / 2)
...
hook.Add("Tick", "Physgun Build Mode:Snap", function()
    ...
    if not r then
        print("Physgun Build Mode Snap Error: " .. e)
    end
end)
...
hook.Add("PhysgunDrop", "Physgun Build Mode:Drop", function(ply, ent)
    ...
    if not r then
        print("[Physgun Build Mode] Drop Error: " .. e)
    end
end)
...
hook.Add("PhysgunPickup", "Physgun Build Mode:Pickup", function(ply, ent)
    ...
    if not r then
        print("[Physgun Build Mode] Pickup Error: " .. e)
    end
end)
